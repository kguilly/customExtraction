...
string filePath = "/home/CodeStuff/data/";   // path to "data" folder. File expects structure to be: 
                                            // .../data/<year>/<yyyyMMdd>/hrrr.<yyyyMMdd>.<hh>.00.grib2
                                            // for every hour of every day included. be sure to include '/' at end
string writePath = "/home/CodeStuff/WRFData/";   //path to write extracted data to, point at a WRF folder

                                                    // point at a WRFData folder
string repositoryPath = "/home/CodeStuff/customExtraction2/";//PATH OF THE CURRENT REPOSITORY
...
//Same as above but for the purpose of using CUDA threads.
struct cudaThreadArgs{
    FILE* f;
    const char* fileName;
    const char* pathName;
    int threadIndex;
    const char* hour;
    const char** charCurrentDay;
};
...
__device__ bool *blnParamArr; // this will be used to quickly index whether a parameter needs to be 
...
int main(int argc, char*argv[]){
    ...
    while(checkDateRange(intcurrentDay, endDay)){
        ...
        const char* charCurrentDay [4];
        for (int i = 0; i < 4; i++){
            charCurrentDay[i] = strCurrentDay.at(i).c_str();
        }

        FILE* f[intHourRange]; // use to open the file for each hour
        threadArgs *arrThreadArgs = new threadArgs[intHourRange];
        cudaThreadArgs *arrCudaThreadArgs = new cudaThreadArgs[intHourRange];
        ...
        for(int i=0;i<intHourRange;i++){ // for each hour, thread the file and filename
            ...
            //Initialized the thread arguments for CUDA threads the same way as pthreads.
            arrThreadArgs[i].f = f[i];
            arrCudaThreadArgs[i].f = fopen(filePath2.c_str(), "rb");  //This is required in main because the GPU cannot open files.
            if(!f){
                printf("Error: could not open filename %s in directory %s", fileName.c_str(), filePath2.c_str());
                exit(0);
            }
            arrThreadArgs[i].fileName = fileName;
            arrCudaThreadArgs[i].fileName = fileName.c_str();
            arrThreadArgs[i].pathName = filePath2;
            arrCudaThreadArgs[i].pathName = filePath2.c_str();
            arrThreadArgs[i].threadIndex = i; 
            arrCudaThreadArgs[i].threadIndex = i; //This may not be necessary if you call the thread index within the thread
            arrThreadArgs[i].hour = hour;
            arrCudaThreadArgs[i].hour = hour.c_str();
            arrThreadArgs[i].strCurrentDay = strCurrentDay;    
            arrCudaThreadArgs[i].charCurrentDay = charCurrentDay;   
            ...
            /*
            CUDA or thread function call would go here.  Haven't made it yet but it would be something like:
            
            cudaMallocManaged((void**)&f, (2*intHourRange*sizeof(arrCudaThreadArgs[i].f)));
            readData <<<1, intHourRange>>> (arrCudaThreadArgs);
            cudaFree(f);
            cudaDeviceSynchronize();

            This may not work, this is just an edited example I pulled from one of the previous versions.
            */

            /*
            This would go after the cuda function call.  Reason being is that mapData should not be too intensive,
            and really would just be a pain to implement in a CUDA threaded function.

            mapData(arrCudaThreadArgs[i].charCurrentDay[3], arrCudaThreadArgs[i].hour, i);
            */

            fclose(arrThreadArgs[i].f);
            fclose (arrCudaThreadArgs[i].f);
...
__global__ 
void readData(cudaThreadArgs* cudaThreadArg){
    
    
    int threadIndex = blockIdx.x * blockDim.x + threadIdx.x;

    FILE *f = cudaThreadArg[threadIndex].f;
    const char* fileName = cudaThreadArg[threadIndex].fileName;
    const char* filePath2 = cudaThreadArg[threadIndex].pathName;
    const char* hour = cudaThreadArg[threadIndex].hour;
    const char ** charCurrentDay = cudaThreadArg[threadIndex].charCurrentDay;

    printf("\nOpening File: %s", filePath2);
    ...
    while((h=codes_handle_new_from_file(0, f, PRODUCT_GRIB, &err))!=NULL) // loop through every layer of the file
    {
        ...
        if (blnParamArr[msg_count] == true){
            ...
            if(!lats){
                printf("Error: unable to allocate %ld bytes\n", (long)(numberOfPoints * sizeof(double)));
                exit(0);
            }
    ...
    //mapData(charCurrentDay[3], hour, threadIndex);
    //pthread_exit(0);

}

void mapData(const char* date, const char* hour, int threadIndex){
    string strDate(date);
    string strHour(hour);
    string hourlyDate = strDate + strHour;
    for (int i = 0; i<numStations; i++){
        Station *station = &stationArr[i];
