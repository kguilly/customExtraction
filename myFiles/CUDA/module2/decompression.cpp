...
TODO: implement binary search in indexing stations
...
main(){
    handle args
    build station array
    pass to gpu to index

    HARD PART: build grib object that tells where to find information

    while there are still hours to extract {
        call function that orchestrates putting files on and off the gpu {
            will put files on gpu as memory frees up, and have its own copy of the stationArr {
                this may mean that station.values should be of size [numDays*numHours][numParams]
            }
        }
    }
