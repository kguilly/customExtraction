...
station_t* orchestrate_grib_decompression(station_t* loc_st_arr, codes_index* grb_idx, FILE* f, char** shortNames,
                                          char** levels, int numStations, int numSelectedParams){
    // copy over all necessary information to the GPU

    // call the kernel
}
__global__ void cuda_decompress_grib(station_t* loc_st_arr, codes_index* grb_idx, FILE* f, char** shortNames,
                                          char** levels, int numStations, int numSelectedParams) {
    
    // find the idx

    // index out the correct shortname and level

    // instantiate the handle obj

    // call the handle new from index subroutine

}

__global__ void handle_new_from_index(FILE* f, codes_index* grb_idx, const char * param_shortname, const char* param_level,
                                      codes_handle* grb_handle) {

    // engineer the codes_handle_new_from_index to operate on a gpu
