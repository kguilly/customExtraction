...
def county(fips=None, write_path=""):
    ...
    for fipscode in fips:
        ...
        # print(grid)
        header = ['countyGridIndex', 'FIPS', 'stateFips', 'county', 'lat (urcrnr)', 'lon (urcrnr)', 'lat (llcrnr)', 'lon (llcrnr)']
        grididx=0
        for i in range(len(grid)):
            ...
            latll = lats[0]
            lonll = lons[0]
            dict[str(grididx) + "_" +fipscode] = [str(grididx), thisfips, stateFips, countyName, latur, lonur, latll, lonll]
            grididx+=1
