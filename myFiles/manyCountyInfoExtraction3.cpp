...
void handleInput(int argc, char*argv[]){
    ...
    buildHours();
    defaultParams(param_flag, param_index, argv);
    readCountycsv();
    matchState();
...
void defaultParams(bool param_flag, int param_index, char* argv[]){
    // if they passed a file, then read the file. If they did not pass a file, then extract the
    // params that fudong needs
    string param_file_dir;
    if(param_flag) {
        param_file_dir = argv[param_index + 1];
    }
    else{
        param_file_dir = repositoryPath + "myFiles/default_parameters.csv";
    }
    ifstream param_file;
    param_file.open(param_file_dir);
    if(!param_file){
        cerr << "Error opening the PARAMETER file that was passed as arg.\n";
        exit(1);
    }
    vector<string> param_row;
    string param_line;
    while(getline(param_file, param_line)){
        stringstream s(param_line);
        string currLine;
        while(getline(s, currLine, ',')){
            param_row.push_back(currLine);
        }
    }
    for(int i=0; i<param_row.size(); i++){
        cout << param_row[i] << endl;
    }
    exit(0);
...
void *readData(void *args){
    ...
    while((h=codes_handle_new_from_file(0, f, PRODUCT_GRIB, &err))!=NULL) // loop through every layer of the file
    {
        ...
        // TODO: only extract the data if the short name matches that of one of the ones passed
        // TODO: or; if no file is passed, extract all parameters
        // TODO: make sure logic to index station is only run once a month
        // extract the data
