...
class formatWRF():
    ...
    def findFiles(self):
        fipsFolders = sorted(os.listdir(self.data_path))
        # print(fipsFolders)
        csvFiles = []
        
        for ff in fipsFolders:
            fipsPath = self.data_path + ff + '/'
            # print(fipsPath)
    ...
    def dailyAvgMinMax(self, df=pd.DataFrame()):
        ...
        '''
        # grouped_day = df.groupby(df.Day)
        # # for each day
        # for i in range(0,32):
        #     try:
        #         daydf = grouped_day.get_group(i)
        #     except:
        #         continue
        #     avgedRow = []
        #     for col in daydf:
        ...
        #             # avgedRow.append(value at the first index of the column)
        #             avgedRow.append(daydf[col].iloc[0])
        #         else:
        #             # get the average of the row and append it to the avg list
        #             avgedRow.append(daydf[col].mean())
        #             # if the column is temperature, find the min, max, then append
        #             if col.rfind("Temperature") != -1:
        #                 avgedRow.append(daydf[col].min())
        #                 avgedRow.append(daydf[col].max())



        # for daytuple in grouped_day:
        #     daydf = 
        #     avgedRow = []
        #     for col in list(dayDf.columns):
        #         if col.rfind('Hour') != -1:
        #             continue
        #         elif col.rfind('Year') != -1 or col.rfind('Month') != -1 or col.rfind('Day') != -1 or col.rfind('State') != -1 or col.rfind('County') != -1 or col.rfind('GridIndex') != -1 or col.rfind('FIPS Code') != -1 or col.rfind('Lat') != -1 or col.rfind('Lon(-180') != -1:
        #             # avgedRow.append(value at the first index of the column)
        #             avgedRow.append(col.iloc[0])
        #         else:
        #             print("idccccccccccc")
        #     # print(dayDf)
        #     mintemps = pd.DataFrame(columns=['tempindex', 'value'])
        #     maxtemps = pd.DataFrame(columns=['tempindex', 'value'])
        #     # keep track of each of the min and max temps
        #     for col in dayDf:
    ...
    def dftocsv(self, df=pd.DataFrame(), fullfilepath=''):
        ...
        newfilename = filename[0:len(filename)-5] + 'averaged.csv'
